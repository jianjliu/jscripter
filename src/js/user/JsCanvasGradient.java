 ...
 * {@link JsClient#CanvasGradient}</tt> class.</p>
 * <p>An object of this opaque class represents a color gradient that may be assigned to 
 * both the {@link JsCanvasRenderingContext2D#strokeStyle} and {@link JsCanvasRenderingContext2D#fillStyle} 
 * properties of a {@link JsCanvasRenderingContext2D#strokeStyle} object. The 
 * {@link JsCanvasRenderingContext2D#createLinearGradient(Number, Number, Number, Number)} 
 * and {@link JsCanvasRenderingContext2D#createRadialGradient(Number, Number, Number, Number, Number, Number)} 
 * methods both return objects of this class.</p>
 * <p>Once you have created an object of this class, use {@link JsCanvasGradient#addColorStop(Number, String)} 
 * method to specify what colors should appear at what positions within the gradient. 
 * Between the positions you specify, colors are interpolated to create a smooth gradient 
 * or fade. Transparent black stops are created implicitly at the start and end points of 
 * the gradient.</p>
 *
 ...
 */
public class JsCanvasGradient extends JsClient.CanvasGradient.Prototype
{
	/**
	 ...
	 * circumstances.
	 */
	public static class Member extends JsClient.CanvasGradient.Prototype.Member
	{
		/**
		 ...
		 */
		public JsCanvasGradient with(ObjectLike o) {
			return new JsCanvasGradient(super.with(o));
		}
	}
	 ...
	 */
	public JsCanvasGradient(JsObject var) {
		super(var);
	}
	 ...
	 */
	public JsCanvasGradient valueOf() {
		return new JsCanvasGradient((JsObject)var().valueOf());
	}
	/**
	 * <p>Specifies a color and position for the gradient.</p>
	 * <p>This method provides the mechanism for describing color changes in a gradient. It 
	 * may be called one or more times to change the color at particular percentages 
	 * between the gradient's start and end points.</p>
	 * <p>If this method is never called on a gradient, the gradient is transparent. At 
	 * least one color stop must be specified to produce a visible color gradient.</p>
	 * @param offset A floating-point value in the range 0.0 to 1.0 that represents a 
	 * fraction between the start and end points of the gradient. An offset of 0 
	 * corresponds to the start point, and an offset of 1 corresponds to the end point.
	 * @param color Specifies the color to be displayed at the specified offset, as a 
	 * CSS color string. Colors at other points along the gradient are interpolated based 
	 * on this and any other color stops.
	 * @since 1.0
	 * @javascript Re-compilers must convert the instance invocation of this method directly
	 * into a JavaScript invocation on its current object instance without changing the 
	 * method name, but expanding variable arguments, if any, into comma-separated values. 
	 */
	public final void addColorStop(Number offset, String color) {
		call(addColorStop, new Vars<Object>().add(offset).add(color));
	}
}
