 ...
 * {@link JsClient#DOMParser} class.</p>
 * <p>A {@link JsDOMParser} object is used to parse XML text and return an XML {@link JsDocument} 
 * node. To use a {@link JsDOMParser} object, instantiate one with the no-argument 
 * {@link JsClient#DOMParser} class and then call its 
 * {@link JsDOMParser#parseFromString(String, String)} method:
 * <pre>JsDocument doc = Js.g().DOMParser.create().parseFromString(text);</pre>
 * </p>
 * <p>Note that a {@link JsXMLHttpRequest} object can also parse XML documents. See the 
 * {@link JsXMLHttpRequest#responseXML} property.</p>
 * <p>IE does not support this class. Instead, it supports XML parsing with 
 * {@link JsDocument#loadXML(String)}.</p>
 *
 ...
 */
public class JsDOMParser extends JsClient.DOMParser.Prototype
{
	/**
	 ...
	 * circumstances.
	 */
	public static class Member extends JsClient.DOMParser.Prototype.Member
	{
		/**
		 ...
		 */
		public JsDOMParser with(ObjectLike o) {
			return new JsDOMParser(super.with(o));
		}
	}
	 ...
	 */
	public JsDOMParser(JsObject var) {
		super(var);
	}
	 ...
	 */
	public JsDOMParser valueOf() {
		return new JsDOMParser((JsObject)var().valueOf());
	}
	/**
	 * <p>Parses XML mark-up and returns a {@link JsDocument} node.</p>
	 * <p>See {@link JsDocument#loadXML(String)} for an IE-specific alternative to this 
	 * method.</p>
	 * @param text The XML mark-up to parse.
	 * @param contentType The content type of the text. This may be one of "text/xml", 
	 * "application/xml", or "application/xhtml+xml". Note that "text/html" is not 
	 * supported by this method.
	 * @return A {@link JsDocument} node that holds the parsed representation of <tt>text</tt>.
	 * @since 1.0
	 * @javascript Re-compilers must convert the instance invocation of this method directly
	 * into a JavaScript invocation on its current object instance without changing the 
	 * method name, but expanding variable arguments, if any, into comma-separated values. 
	 */
	public final JsDocument parseFromString(String text, String contentType) {
		return new JsDocument(call(parseFromString, new Vars<Object>().add(text).add(contentType)));
	}
}
