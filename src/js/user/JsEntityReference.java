...
/**
 * <p>An <b>opaque</b> class representing JavaScript client-side <tt>EntityReference</tt> nodes.</p>
 * <p>{@link JsEntityReference} objects may be inserted into the structure model when an 
 * entity reference is in the source document, or when the user wishes to insert an 
 * entity reference. Note that character references and references to predefined entities 
 * are considered to be expanded by the HTML or XML processor so that characters are 
 * represented by their Unicode equivalent rather than by an entity reference. Moreover, 
 * the XML processor may completely expand references to entities while building the 
 * structure model, instead of providing {@link JsEntityReference} objects. If it does 
 * provide such objects, then for a given {@link JsEntityReference} node, it may be that 
 * there is no {@link JsEntity} node representing the referenced entity. If such a {@link JsEntity} 
 * node exists, then the subtree of the {@link JsEntityReference} node is in general a 
 * copy of the {@link JsEntity} node subtree. However, this may not be true when an 
 * entity contains an unbound name space prefix. In such a case, because the name space 
 * prefix resolution depends on where the entity reference is, the descendants of the 
 * {@link JsEntityReference} node may be bound to different namespace URIs.</p>
 * <p>As for {@link JsEntity} nodes, {@link JsEntityReference} nodes and all their 
 * descendants are read-only.</p>
 * <p>Note that main-stream web browsers do not support the <tt>EntityReference</tt> interface 
...
public class JsEntityReference extends JsNode
{
	...
	public static class Member extends JsClient.Node.Prototype.Member
	{
		 ...
		 */
		public JsEntityReference with(ObjectLike o) {
			return new JsEntityReference(super.with(o));
		}
	}
	 ...
	 */
	public JsEntityReference(JsObject var) {
		super(var);
	}
	 ...
	 */
	public JsEntityReference valueOf() {
		return new JsEntityReference((JsObject)var().valueOf());
	}
}
