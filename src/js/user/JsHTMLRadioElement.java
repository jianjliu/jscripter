 ...
 * {@link JsClient#HTMLRadioElement} class.</p>
 * <p>This class represents a push button 
 * in an HTML page, or a {@link JsHTMLInputElement} object created with the HTML 
 * <tt>&lt;input&gt;</tt> tag with its <tt>type</tt> 
 * attribute of the value "radio".</p>
 * <p>An input element of this class displays a single graphical radio button. A radio 
 * button is a button in a group of buttons that represents a set of mutually exclusive 
 * choices. When one button is selected, the previously selected button is deselected, 
 * as in the mechanical station preset buttons of old car radios. In order for a group 
 * of radio buttons to exhibit this mutually exclusive behavior, they must appear in the 
 * same {@link JsHTMLInputElement#form} and must have the same {@link JsHTMLInputElement#name}. 
 * For toggle buttons without mutual exclusion, use a type of "checkbox" or {@link JsHTMLCheckBoxElement}. 
 * Note that the HTML <tt>&lt;select&gt;</tt> tag creating {@link JsHTMLSelectElement} 
 * can also be used for presenting exclusive or non-exclusive choices.</p>
 * <p>The {@link JsHTMLInputElement#checked} property indicates whether a radio button 
 * is selected. There is no way to determine which button in a mutually exclusive group 
 * of radio buttons is selected. You must examine the {@link JsHTMLInputElement#checked} 
 * property of each one. Radio buttons trigger the {@link JsHTMLInputElement#onclick} 
 * event handler when selected or deselected.</p>
 * <p>The {@link JsHTMLInputElement#value} property specifies a value to be submitted 
 * to a web server and is not displayed within the form. To specify a label for a radio 
 * button, do so externally to the <tt>&lt;input&gt;</tt> tag, such as with a <tt>&lt;label&gt;</tt> 
 * tag.</p>
 *  
 ...
 */
public class JsHTMLRadioElement extends JsClient.HTMLRadioElement.Prototype
{
	/**
	 ...
	 * circumstances.
	 */
	public static class Member extends JsClient.HTMLRadioElement.Prototype.Member
	{
		/**
		 ...
		 */
		public JsHTMLRadioElement with(ObjectLike o) {
			return new JsHTMLRadioElement(super.with(o));
		}
	}
	 ...
	 */
	public JsHTMLRadioElement(JsObject var) {
		super(var);
	}
	 ...
	 */
	public JsHTMLRadioElement valueOf() {
		return new JsHTMLRadioElement((JsObject)var().valueOf());
	}
}
