 ...
 * {@link JsClient#XPathExpression} class.</p>
 *
 ...
 */
public class JsXPathExpression extends JsClient.XPathExpression.Prototype
{
	/**
	 ...
	 * circumstances.
	 */
	public static class Member extends JsClient.XPathExpression.Prototype.Member
	{
		/**
		 ...
		 */
		public JsXPathExpression with(ObjectLike o) {
			return new JsXPathExpression(super.with(o));
		}
	}
	 ...
	 */
	public JsXPathExpression(JsObject var) {
		super(var);
	}
	 ...
	 */
	public JsXPathExpression valueOf() {
		return new JsXPathExpression((JsObject)var().valueOf());
	}
	/**
	 * <p>Evaluates this expression for a specified context node.</p>
	 * @param contextNode The node or document against which the query should be evaluated.
	 * @param type The desired result type.
	 * This argument should be one of the constants defined by {@link JsClient#XPathResult}.
	 * @param result A {@link JsXPathResult} object into which the results of the query 
	 * should be stored, or <tt>null</tt> to have this method create and return a new 
	 * {@link JsXPathResult} object.
	 * @return A {@link JsXPathResult} object that hold the results of the query. This 
	 * is either the object passed as the result argument or a newly created {@link JsXPathResult} 
	 * object if result was <tt>null</tt>.
	 * @since 1.0
	 * @see JsXPathResult
	 * @javascript Re-compilers must convert the instance invocation of this method directly
	 * into a JavaScript invocation on its current object instance without changing the 
	 * method name, but expanding variable arguments, if any, into comma-separated values. 
	 */
	public final JsXPathResult evaluate(JsNode contextNode, Number type, JsXPathResult result) {
		return new JsXPathResult(call(evaluate, new Vars<Object>().add(contextNode).add(type).add(result)));
	}
	/**
	 * <p>Evaluates this expression for a specified context node.</p>
	 * @param contextNode The node or document against which the query should be evaluated.
	 ...
	 * method name, but expanding variable arguments, if any, into comma-separated values. 
	 */
	public final JsXPathResult evaluate(JsNode contextNode, Number type) {
		return new JsXPathResult(call(evaluate, new Vars<Object>().add(contextNode).add(type).add(new JsXPathResult(new JsObject()))));
	}
	/**
	 * <p>Evaluates this expression for a specified context node.</p>
	 * @param contextNode The node or document against which the query should be evaluated.
	 ...
	 * method name, but expanding variable arguments, if any, into comma-separated values. 
	 */
	public final JsXPathResult evaluate(JsNode contextNode) {
		return new JsXPathResult(call(evaluate, new Vars<Object>().add(contextNode).add(JsClient.XPathResult.ANY_TYPE.with()).add(new JsXPathResult(new JsObject()))));
	}
}
