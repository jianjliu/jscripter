...
/**
 * <p>An <b>internal</b> class for JS Embeded Simulation on IE browser series.</p>
 ...
 */
public final class EmbededIE extends Embeded
{
	/**
	 * <p>Internally constructs a JS Embeded Simulation from an applet on an IE browser.</p>
	 * @param applet A Java applet.
	 * @since 1.0
	 * 
	 * @javascript Re-compilers must report error on the invocation to an <b>internal</b> constructor. 
	 */
	protected EmbededIE(Applet applet) {
		super(applet);
		getWindow().eval(
				"window.$$set=function(o,p,v){o[p]=v;};" +
				"alert0=window.alert;" +
				"window.alert=function(s){" +
					"return alert0(s);" +
				"};" +
				"setTimeout0=window.setTimeout;" +
				"window.setTimeout=function(f,d){" +
					"return setTimeout0(f,d);" +
				"};" +
				"clearTimeout0=window.clearTimeout;" +
				"window.clearTimeout=function(i){" +
					"return clearTimeout0(i);" +
				"};" +
				"setInterval0=window.setInterval;" +
				"window.setInterval=function(f,i){" +
					"return setInterval0(f,i);" +
				"};" +
				"clearInterval0=window.clearInterval;" +
				"window.clearInterval=function(i){" +
					"return clearInterval0(i);" +
				"};" +
				"window.$cpe=function(){" +
					"return $cp(event);" +
				"};"
	...
	protected <T> JsFunction<T> var(Function<T> f) {
		return new JseFunction<T>(
				new JseObjectIE.JseoFunction<T>(
						f
				)
		);
	}
	...
	protected JsWin win() {
		if (win == null) {
			win = new JsWin(new JseObjectIE(getWindow()));;
		}
		return win;
	}
	...
	protected Vars<?> vars(Object args) {
		...
		if (args instanceof JSObject) {
			if (((JSObject)args).getMember("length") != null) {
				args = new JseArray(new JseObjectIE(args));
			}
		}
		if (args instanceof ArrayObject<?>) {
			ArrayObject<?> a = (ArrayObject<?>)args;
			for (int l = a.length(), i = 0; i < l; i++) {
				vars.add(JseObjectIE.wrap(a.get(i)));
			}
		} else if (args.getClass().isArray()) {
			Object[] a = (Object[])args;
			for (int l = a.length, i = 0; i < l; i++) {
				vars.add(JseObjectIE.wrap(a[i]));
			}
		} else if (args != null){
			vars.add(JseObjectIE.wrap(args));
		}
		return vars;
	}
	...
	protected final ArrayLike<String> keys(JsObject o) {
		return (ArrayLike<String>)(ArrayLike<?>)new JseArray(new JseObjectIE(
				getWindow().call(
						"$keys",
